...
{
QRect rt = this->rect();
QPoint tl = mapToGlobal(rt.topLeft());
QPoint rb = mapToGlobal(rt.bottomRight());
...
//左边
else if (x <= tl.x() + m_padding && x >= tl.x() - m_padding && y > tl.y() && y < rb.y())
{
    m_direct = LEFT;
...
//右边
else if (x <= rb.x() + m_padding && x + m_padding >= rb.x() && y > tl.y() && y < rb.y())
{
...
void FramelessWin::keyPressEvent(QKeyEvent *evt)
{
    switch ( evt->key())
    {
        case Qt::Key_Escape:
